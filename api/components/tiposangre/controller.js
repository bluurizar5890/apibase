const { TipoSangre } = require('../../../store/db');
...
const update = async (req) => {
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(tabla, tipo_sangreId, dataAnterior.dataValues, req.body);
...
const eliminar = async (req) => {
    ...
    let tipo_sangreId = req.params.id;
    const dataAnterior = await Modelo.findOne({
        where: { tipo_sangreId }
    });
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(tabla, tipo_sangreId, dataAnterior.dataValues, dataEliminar);
